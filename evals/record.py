...
class LocalRecorder(RecorderBase):
    ...
    def record_final_report(self, final_report: Any):
        ...
        print(
            f"--- {self.run_spec.setting_name}  Final report: {final_report}. Logged to {self.event_file_path}"
        )

        filename = f"/tmp/res/{self.run_spec.completion_fns[0]}.json"
        print(f"--- Score has been updated in the JSON file for {self.run_spec.completion_fns[0]}.")
        print(f"File location: {filename}")
        import json
        from pathlib import Path

        path = f"/tmp/res/{self.run_spec.completion_fns[0]}.json"
        p = Path(path)
        ...
        if os.path.exists(path):
            ...
            loaded_data = {}
        loaded_data[self.run_spec.setting_name] = (final_report, self.event_file_path)
